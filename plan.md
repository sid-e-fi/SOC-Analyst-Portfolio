...
| 2 | Migrate & clean Security Fundamentals notes | NOT STARTED — blocked on Phase 1 ✅ unblocked; needs the 14 source note files from you |
| 3 | Build the glossary | NOT STARTED — blocked on Phase 2 |
...
**Next action: Phase 1 is complete. Phase 2 needs the 14 Security Fundamentals source note files (from the SOC-L1 vault backup) uploaded before it can run.**
...
| 5 | All 14 Security Fundamentals notes are preserved in full — none are redundant enough to cut, each covers a distinct SOC-relevant topic. | "Preserve knowledge first, organize second, polish third." | `plan.md` |
...
|---|---|---|
| `Notes/Obsidian Vault/01 Security Fundamentals/` | 14 concept/course notes, all `status: reviewed` | Real content — ready to migrate |
...
**Total real, usable content:** 14 concept notes + 1 glossary + 6 daily logs + 1 weekly summary + 3 lab docs ≈ 12,450 lines of Markdown, and 13 screenshots (~27 MB), all from Week 1. Everything else is empty future scaffolding.

### Content quality assessment
- All 14 Security Fundamentals notes carry consistent YAML frontmatter (`type`, `status: reviewed`, `tags`), a purpose blockquote, and a "Related Notes" wikilink section — a genuinely consistent note system, not a random dump. Topics: CIA Triad, Risk Management, IAM, Authentication & Authorization, Identity Attacks, Malware & Social Engineering, Network Security, Vulnerabilities & Patch Management, Threat Actors & Cyber Warfare, Defense in Depth, Incident Response, Cryptocurrency & Cryptojacking, a general Cybersecurity Fundamentals note, a mental-models note, and Cisco course notes.
...
### Remaining (yours to do)
Run the commands above (or apply via GitHub's web UI) to actually push the scaffold. Let me know once it's pushed, or just say "continue" and I'll proceed to Phase 2 regardless — push confirmation isn't a hard blocker for planning the next phase, only for anything that depends on the skeleton already being live upstream.

### Potential risks/problems
- None encountered. The rename was already done and the repo was empty, so this was a clean scaffold with no conflicting content to reconcile.

### Next Phase
Phase 2: Migrate & Clean Security Fundamentals Notes — **needs the 14 source note files from the `01 Security Fundamentals/` vault folder uploaded before it can run.**

---

## Phase 2: Migrate & Clean Security Fundamentals Notes

**Status: NOT STARTED — Phase 1 ✅ complete; blocked on receiving the 14 source note files from you**

### Objective
Move the 14 reviewed notes from `01 Security Fundamentals/` into `notes/security-fundamentals/`, renamed to kebab-case, with frontmatter handled per the Phase 1 decision and wikilinks converted to relative Markdown links.

### Why it exists
This is the largest chunk of real, demonstrable learning content in the portfolio.

### Prerequisites
Phase 1 complete (folder skeleton exists, frontmatter decision confirmed).

### Files required from you
None beyond what's already in the uploaded backup, unless any of these 14 notes have been revised since — provide updated versions if so.

### Files Claude will create/change
All 14 notes, renamed and moved into `notes/security-fundamentals/` (see file list in the merged structure above).

### Specific tasks
1. Copy each note's content verbatim — no rewriting of meaning or technical content.
2. Rename per kebab-case convention.
3. Apply the confirmed frontmatter treatment consistently across all 14 files.
4. Convert every `[[Wikilink]]` to a relative Markdown link pointing at the new kebab-case filename. Every current target resolves to a real file in this batch except the one broken link (handled where it actually appears — check whether it's reused in `week-1-summary.md`, since raw daily logs no longer ship).
5. Convert the one internal section-anchor link (`CIA Triad...`'s `[[#Vulnerability vs Risk]]`) to a GitHub-compatible Markdown anchor.
6. Do not alter technical content or wording beyond what link/frontmatter/filename conversion requires. Flag, don't silently fix, any technical claim that looks questionable.
7. Update `plan.md`.

### Expected output
14 clean, cross-linked Markdown files in `notes/security-fundamentals/`, all links resolving on GitHub.

### Validation criteria
Every relative link resolves; no technical content altered in meaning; frontmatter treatment consistent across all 14 files.

### Completion criteria
All 14 files migrated, validated, committed (or handed off as commands per Phase 1's delivery decision).

### Potential risks/problems
If the frontmatter decision looks worse once actually applied, flag it rather than silently reverting.

---

## Phase 3: Build the Glossary

**Status: NOT STARTED — blocked on Phase 2 (Conflict #2 already resolved: root location)**
